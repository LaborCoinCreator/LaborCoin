[LaborCoin-Technical-Whitepaper-v1.0-economic-flow-updated.md](https://github.com/user-attachments/files/29432519/LaborCoin-Technical-Whitepaper-v1.0-economic-flow-updated.md)
Version: 1.0
...
| Figure 2  | User Journey Overview                      |
...
| Figure 8  | Detailed Participant Lifecycle             |
...
Illustrates the intended governance-controlled process through which treasury resources move from accumulation to approved distribution. Exclusivity of this path depends upon the final Aragon DAO permission registry and removal of obsolete executors.
...
The LaborCoin Exchange addresses this through continuous contract-managed liquidity.
...
The LaborCoin Exchange provides continuous contract-managed liquidity through a deterministic quadratic bonding curve.
...
Exchange V4 rejects a calculated LABR price above 100 POL per LABR, providing a bounded oracle-anomaly check.
...
The verifier cannot directly mint LABRV, create a proposal, cast a vote, or execute a treasury transfer. It authorizes registration eligibility or a governance action category; the contracts perform the state changes only after all on-chain checks pass.
...
Governance V13 maintains a nonce for each participant and requires action-code-specific verifier signatures for proposal creation and voting. The signature authorizes the action category rather than a particular proposal ID or proposal contents.

A valid governance authorization must contain the expected action code and nonce, identify Governance V13, and remain within its expiration deadline. After successful use, Governance V13 increments the participant's nonce. A reused signature therefore fails even if its expiration time has not yet passed.
...
Proposal and vote authorizations use action codes, participant nonces, expiration timestamps, and the Governance V13 contract address.
...
A calculated LABR price above 100 POL per LABR is rejected.
...
Large sell pressure exceeds Exchange V4's available POL balance.
...
A sale succeeds only when Exchange V4 holds enough POL to cover the calculated payout.
...
flowchart TD
    ...
    LABRV --> Govern[Create Proposals and Vote with Verifier Authorizations]
...
Participants create or connect a Human Passport profile and accumulate sufficient verification signals to satisfy the protocol threshold.
...
The participant address must not already be recorded as registered. LaborVote V7 also prevents minting an additional LABRV to an address that already holds one.
...
Under the standard onboarding path, successful registration results in issuance of:

$$
1 	ext{ LABRV}
$$

The governance token is minted directly to the participant's wallet. If a previously unregistered wallet already holds LABRV, Registration V4 records the registration without minting another token.

Governance V13 checks LABRV ownership directly and records one vote for each eligible registered wallet. ERC20Votes delegation is not required for participation in Governance V13.
...
Once registered and holding LABRV, participants may engage in governance activities. Proposal creation and voting also require current verifier authorizations containing a nonce and expiry.
...
Any address may submit an approved proposal for execution during its valid execution window. Governance V13 uses the stored proposal terms and revalidates all execution conditions.
...
If the participation and approval thresholds are satisfied, the proposal becomes vote-approved.

Execution may then be attempted during the execution window. Governance V13 separately checks the fifty-member activation requirement, the five-percent cap based on the DAO's current native POL balance, the execution deadline, and prior-execution status.
...
Approved treasury distributions combine two separate processes:

1. **Economic value accumulation**, through which assets enter the Aragon DAO treasury.
2. **Governance authorization**, through which Governance V13 determines whether a constrained POL transfer may be submitted to the DAO for execution.

Treasury assets do not flow into Governance V13. Governance V13 does not custody POL, LABR, exchange liquidity, or proposal funds.

The Aragon DAO remains the treasury custodian. Governance V13 records proposals and votes, evaluates fixed requirements, and constructs the approved DAO action using the stored recipient and amount.
...
The two DAO nodes in the diagram represent different aspects of the same Aragon DAO:

* **Aragon DAO Treasury** represents custody of protocol assets.
* **Aragon DAO Execute** represents the permission-controlled execution function used to carry out the approved action.

A successful Governance V13 distribution follows this sequence:

1. A LABRV holder creates a treasury proposal using a valid verifier authorization.
2. Eligible LABRV holders vote during the fourteen-day voting period.
3. Governance V13 evaluates participation and approval.
4. During the seven-day execution window, any address may call `executeProposal`.
5. Governance V13 revalidates the fifty-member activation requirement, the execution deadline, prior-execution status, and the five-percent cap against the DAO's current native POL balance.
6. Governance V13 constructs the fixed DAO action using the stored recipient and amount.
7. The Aragon DAO supplies the approved POL to Treasury Module V1 as call value.
8. Treasury Module V1 forwards that POL to the approved recipient.
9. Governance V13 marks the proposal executed, and Treasury Module V1 updates its cumulative distribution accounting.

Governance V13 proposals distribute native POL only. LABR or other assets held by the DAO are outside the Governance V13 treasury-transfer proposal format.

Under the final intended Aragon permission configuration, this path ensures that protocol treasury distributions originate from approved Governance V13 decisions while treasury custody remains with the Aragon DAO.
...
The LaborCoin Exchange provides continuous contract-managed liquidity.
...
| Treasury Drain         | 5% of DAO Native POL Balance at Execution |
...
* Progressive scarcity
* Continuous contract-managed liquidity
...
| Duplicate Registration    | Registration-State Check + LABRV Mint Balance Check | Registration |
...
| Treasury Extraction       | 5% of DAO Native POL Balance at Execution | Treasury     |
