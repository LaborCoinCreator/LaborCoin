...
contract LaborCoin is ERC20, ERC20Burnable, Ownable2Step, Pausable, ERC20Permit, DividendTrackerFunctions, Initializable {
    ...
    uint256 private _solidarityPending;
    uint256 private _rewardsPending;

    address public solidarityAddress;
    uint16[3] public solidarityFees;
    ...
    constructor()
        ERC20(unicode"LaborCoin", unicode"LABR")
        Ownable(msg.sender)
        ERC20Permit(unicode"LaborCoin")
    {
        assembly { if iszero(extcodesize(caller())) { revert(0, 0) } }

        address supplyRecipient = 0x015b6D0990E56D908c876474C6A30eBa2b8A0CFB;
        
        updateSwapThreshold(50);

        solidarityAddressSetup(0x0C2e5679153593b82a84eAB5CA90895BB291Cec4);
        solidarityFeesSetup(0, 250, 0);

        autoBurnFeesSetup(0, 1, 0);

        _deployDividendTracker(86400, 10 * (10 ** decimals()) / 10, 2300);

        gasForProcessingSetup(300000);
        rewardsFeesSetup(0, 250, 0);
        ...
        updateMaxBuyAmount(5000000 * (10 ** decimals()) / 10);
        ...
        _transferOwnership(0x015b6D0990E56D908c876474C6A30eBa2b8A0CFB);
    }
    ...
    function getAllPending() public view returns (uint256) {
        return _solidarityPending + _rewardsPending;
    }

    function solidarityAddressSetup(address _newAddress) public onlyOwner {
        if (_newAddress == address(0)) revert InvalidTaxRecipientAddress(address(0));
        solidarityAddress = _newAddress;
        excludeFromFees(_newAddress, true);
        _excludeFromLimits(_newAddress, true);
        emit WalletTaxAddressUpdated(1, _newAddress);
    }

    function solidarityFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        totalFees[0] = totalFees[0] - solidarityFees[0] + _buyFee;
        totalFees[1] = totalFees[1] - solidarityFees[1] + _sellFee;
        totalFees[2] = totalFees[2] - solidarityFees[2] + _transferFee;
        if (totalFees[0] > 2500 || totalFees[1] > 2500 || totalFees[2] > 2500) revert CannotExceedMaxTotalFee(totalFees[0], totalFees[1], totalFees[2]);
        solidarityFees = [_buyFee, _sellFee, _transferFee];
        ...
        override
    {
        ...
        if (from != address(0) && to != address(0)) {
            if (!_swapping && amount > 0 && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
                ...
                uint256 fees = amount * totalFees[1] / 10000;
                amount -= fees;

                _solidarityPending += fees * solidarityFees[1] / totalFees[1];
                _rewardsPending += fees * rewardsFees[1] / totalFees[1];
        ...
        whenNotPaused
    {
        ...
        if (!tradingEnabled && from != address(0) && to != address(0)) revert TradingNotEnabled();
    }
